        ...
        <div v-else class="table-responsive">
          <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
              <tr>
                <th class="small fw-semibold ps-4">Nº</th>
            ...
            <tbody>
              <tr v-for="item in listaFiltrada" :key="item.id">
                <td class="ps-4"><span class="badge bg-primary-subtle text-primary fw-bold">{{ item.numero }}</span></td>
...
const listaFiltrada = computed(() =>
  lista.value.filter(c => !busca.value || c.descricao?.toLowerCase().includes(busca.value.toLowerCase()))
)
